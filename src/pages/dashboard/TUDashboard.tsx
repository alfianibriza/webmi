...
export default function TUDashboard() {
    ...
    // Data Staff TU (Fallback to user data)
    const staffProfile = {
        name: user?.name || "Staff Tata Usaha",
        role: "Tata Usaha",
        nip: user?.nip || "-",
    ...
    const menuItems = [
        { id: 1, title: "Post Berita", icon: <FileText size={24} />, gradient: "from-blue-400 to-cyan-400", shadow: "shadow-blue-200", path: "/dashboard/post" },
            ...
            <div className="pt-12 px-6 pb-6 flex items-center justify-between relative z-50">
                <div className="flex items-center gap-4">
                    ...
                    <div className="relative">
                        <div className="w-14 h-14 rounded-2xl p-0.5 bg-teal-100 border border-teal-200">
                            <img src={staffProfile.photoUrl} alt="Profile" className="w-full h-full object-cover rounded-[14px]" />
                        </div>
                    </div>
                ...
                <div className="relative">
                    <button
                        ...
                        className="w-10 h-10 rounded-full bg-white text-teal-500 border border-gray-100 flex items-center justify-center shadow-sm hover:shadow-md transition-all relative z-10"
                    >
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-8">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Manajemen akademik & kesiswaan dinonaktifkan.</p>
                        </div>
                    </div>
                    <p className="text-sm text-gray-600 leading-relaxed">
                        Anda dapat mengelola berita dan media melalui menu di bawah ini.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-4 gap-x-4 gap-y-6 mb-8">
                    {menuItems.map((item) => (
                        <Link to={item.path} key={item.id} className="flex flex-col items-center gap-2 group">
                            ...
                            <span className="text-[11px] text-gray-600 text-center font-medium leading-tight px-1 group-hover:text-teal-600 transition-colors">{item.title}</span>
                        </Link>
                    ))}
                </div>
            </div>
